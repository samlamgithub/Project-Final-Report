...
\begin{verbatim}
lsd slam: \footnote{https://github.com/tum-vision/lsd_slam} 
\end{verbatim}


engel2014lsd schops2014semi
...
ll experiments are conducted on a Sony Xperia Z1, which is
equipped with a 2.3 GHz quad-core CPU. While the processing
power of mobile devices has increased rapidly in the last years,
mobile processors based on the ARM architecture are generally
still much slower than their desktop counterparts; we currently do
not use GPU or DSP features for tracking or mapping. In order
to achieve real-time performance, i.e. tracking with at least 30 fps
under these conditions, two steps were crucial: (1) separation of
mapping and tracking resolution and choice of a suitable compromise,
and (2) NEON optimization of computation-heavy algorithmic
steps.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/lsdslam2}
    \caption{lsdslam2}
    \label{fig: lsdslam2}
\end{figure}
